...
| 2026-08-28 | 8 remaining beds | natural | 2 mo | **E37 — PRE-REGISTERED before the sweeps land.** Twice this session an anomaly got a story the next run contradicted (bddp03's σ bug, bddp07's inert licence), so the prediction goes on the record first. **Hypothesis:** the target route beats the AF route where the donor spends more time high, because a larger shift is tolerable when there is more high to correct. **Discriminator: % of counterfactual time ≥180.** Confirmed so far at two points — target wins on bddp05 (52.0 %), loses on bddp11 (30.6 %). **The discriminating test is bddp01 at 45.1 %**: a lows-free runs-high donor where the AF licence gives +1.4 TIR. If the target route beats that, the hypothesis survives; if it is neutral or worse there, % time high is not the discriminator **and I should not retro-fit a different one** | **REFUTED.** No monotone relationship with time-above-180: cht20's largest lift is **bddp06 at 11.5 %** (near the *bottom* of the axis) and its only negative is **bddp03 at 12.2 %**, adjacent on the same axis and opposite in outcome; bddp05 (52 %) and bddp10 (24.9 %) are both strongly positive. **Per the pre-registration I am not fitting a replacement discriminator** — any variable found now would be selected *because* it separates those two adjacent beds, which is the move the pre-registration existed to prevent. **The actuator choice is currently unexplained.** And the test I nominated was unavailable: **bddp01 is DEGENERATE-REF (lows@op 0.000)**, the same reason it is out of the headline — I should have checked its guard status, which was already recorded, before naming it. Descriptively: cht20 > cht10 on 6/9, cht10 ≥ AF on 6/9, cht20 holds both the largest lifts and the only negative; bddp06's +0.0758 deserves caution as a 93 %-TIR donor whose 90-day reference the guard rejected. **C31 stays a per-patient option and the temp-basal-only route — not an established better default** |
