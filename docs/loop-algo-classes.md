...
| D5 | **`decayEffect` continuity** (#33 / LoopKit#556, 2026-05) | RC effect timeline switched from step-by-step accumulation to a closed-form quadratic; identical for delta-aligned samples, differs for **unaligned (real-CGM) timestamps** | `--legacy-rc-decay` | small: mean \|ΔRC\| ~0.025 mg/dL, eventualBG tail ~23 mg/dL on unaligned + large-discrepancy cycles; **improves temp-basal aggregate** (bddp11 within-0.1 80→85%, meanΔ +0.039→+0.023) |
...
| RC `decayEffect`: continuous-quadratic (#33) → **step-by-step** (Loop-main pre-#33) | `--legacy-rc-decay` |
